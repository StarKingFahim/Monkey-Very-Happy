
var monkey , monkey_running, monkeyCollide;

var ground, invisiGround, groundImg;

var back1,back2,backImg;

var banana ,bananaImage, obstacle, obstacleImage;

var FoodGroup, obstacleGroup;

var logo,logo_Img,play,play_Img;

var score = 0;

var bananaScore = 0;

var SERVE= -1;
var PLAY = 0;
var END = 1;

var gameState = SERVE;

function preload()
{
  monkey_running = loadAnimation("sprite_0.png","sprite_1.png","sprite_2.png","sprite_3.png","sprite_4.png","sprite_5.png","sprite_6.png","sprite_7.png","sprite_8.png")
  
  monkeyCollide = loadAnimation("sprite_1.png");
  
  
  groundImg = loadAnimation("ground.png");
  
  backImg = loadImage("skyhill.png");
  
  bananaImage = loadImage("banana.png");
  
  obstacleImage = loadImage("obstacle.png");
  
  logo_Img = loadImage("monkey happy logo.webp");
  
  play_Img = loadImage("play.png");
 
}

function setup(){
 createCanvas(600,300);
  
  obstacleGroup = createGroup();
  bananaGroup = createGroup();
  
 
  monkey = createSprite(80,230,10,10);
  monkey.scale = 0.12;
  monkey.addAnimation("monkey", monkey_running);
  monkey.addAnimation("collide", monkeyCollide);
  
  back1 = createSprite(225,210,10,10);
  back2 = createSprite(400,210,10,10);
  back1.addImage(backImg);
  back2.addImage(backImg);
  back1.scale=1.4;
  back2.scale=1.4;
  back1.depth=0;
  back2.depth=-1;
  
    
  ground = createSprite(450,350,600,10);
  ground.scale = 3;
  
  ground.addAnimation("ground", groundImg);
  
  invisiGround = createSprite(300,278,600,7);
  invisiGround.visible = false;
  
  logo= createSprite(300,100,10,10);
  logo.addImage(logo_Img);
  logo.visible=true;
  play= createSprite(300,225,10,10);
  play.addImage(play_Img);
  play.scale=0.5;
  play.visible=true;
  
  
}

function draw(){
  background("skyblue");
 
      
  
  if(gameState === SERVE)
    {
      
 if (mousePressedOver(play)) 
    {
      gameState=PLAY;         
    }
  
          
      
    }
  
 
  
  if (gameState === PLAY)
  {
    
   logo.visible=false;
   play.visible=false; 

    
    
    obstacles();
    bananas();
    
    fill("black");
    text("SURVIVAL TIME: "+score, 10,20);
    text("Score: "+bananaScore,300,20);
    
    score = score + Math.round(getFrameRate()/60);
    
    ground.velocityX = -(4+score*1.5/100);
  
    if(keyDown("space")&&monkey.y >= 235) {
      monkey.velocityY = -13; 
    }
  
    monkey.velocityY = monkey.velocityY + 0.8
  
    if (ground.x < 120){
      ground.x = ground.width/2;
    }
    
    if (monkey.isTouching(bananaGroup)){
      bananaScore++;  
      bananaGroup.destroyEach();
    
    }
    
    if (monkey.isTouching(obstacleGroup)){
      gameState = END;
    }
    
  }
  
  if (gameState === END)
  {
    
      back1.visible=false;
      back2.visible=false;

    
    ground.velocityX = 0;
    
    monkey.y = 235;
    monkey.scale = 0.12;
    monkey.changeAnimation("collide", monkeyCollide);
    
    obstacleGroup.setVelocityXEach(0);
    
    bananaGroup.setVelocityXEach(0);
    
    obstacleGroup.setLifetimeEach(-1);
    
    bananaGroup.setLifetimeEach(-1);
    
    fill("red")
    stroke("black")
    textSize(30);
    text("GAMEOVER!!!", 220, 170);
    fill("black");
    textSize(15);
    text("Press 'R' to play again", 240, 200);
    
    if (keyDown("r")){
      bananaGroup.destroyEach();
      
      obstacleGroup.destroyEach();
      
      monkey.changeAnimation("monkey", monkey_running);
      
      score = 0;
      
      bananaScore = 0;
      
      back1.visible=true;
      back2.visible=true;
      
      gameState = PLAY; 
    }
  }
  
  
  
  drawSprites(); 
  
  monkey.collide(invisiGround);
}

function bananas(){
  if (frameCount%80 === 0){
    
    banana = createSprite(620,120, 50, 50 )
    banana.addAnimation("banana", bananaImage);
    banana.scale = 0.1;
    banana.velocityX =-(4+score*1.5/100);           
    banana.lifetime = 220;
    bananaGroup.add(banana);
    bananaGroup.add(banana);

    
  }
  

  
}

function obstacles(){
  if (frameCount%200 === 0){
    
    obstacle = createSprite(620,253,50,50);
    obstacle.addAnimation("rock", obstacleImage);
    obstacle.setCollider("circle", 0, 0, 180);
    obstacle.scale = 0.13 ;
    obstacle.velocityX = -(4+score*1.5/100);
    obstacle.lifetime = 220;
    obstacleGroup.add(obstacle);
    
  }
  
  
}






